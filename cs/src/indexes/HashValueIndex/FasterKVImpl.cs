    ...
    internal unsafe partial class FasterKVHVI<TPKey> : FasterKV<TPKey, RecordId>
    {
            ...
            where FasterSession : IFasterSession<TPKey, RecordId, TInput, TOutput, TContext>
        {
            ...
            if (logicalAddress >= hlog.SafeReadOnlyAddress)
            {
                ...
                fasterSession.ConcurrentReader(ref queryKeyPointerRefAsKeyRef, ref input, ref hlog.GetValue(recordPhysicalAddress), ref output, ref pendingContext.recordInfo, logicalAddress);
                return OperationStatus.SUCCESS;
            }
...
#region Create new record in the mutable region
            CreateNewRecord:
            {
                ...
                RecordInfo.WriteInfo(ref hlog.GetInfo(newPhysicalAddress), sessionCtx.version,
                                     ...
                                     tombstone: false, invalidBit:true,
